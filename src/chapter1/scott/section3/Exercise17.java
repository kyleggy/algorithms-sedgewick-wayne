...
public class Exercise17 {
    ...
    public static Exercise16.Date[] readDates() {
        Queue<Exercise16.Date> dateQueue = new Queue<>();
        while(!StdIn.isEmpty()) {
            String stdInDate = StdIn.readString();
            Exercise16.Date date = new Exercise16.Date(stdInDate);
            dateQueue.enqueue(date);
        }
        Exercise16.Date[] returnedDates = new Exercise16.Date[dateQueue.size()];
    ...
    public static Transaction[] readTransactions() {
        ...
        while (!StdIn.isEmpty()) {
            String transactionStr = StdIn.readLine();
            Transaction transaction = new Transaction(transactionStr);
            transactionQueue.enqueue(transaction);
        }
        Transaction[] returnedTransactions = new Transaction[transactionQueue.size()];
        for (int i = 0; i < returnedTransactions.length; i ++) {
            returnedTransactions[i] = transactionQueue.dequeue();
        }
